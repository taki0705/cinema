...
const ShowTimes = () => {
  ...
  const handleBooking = (scheduleId, movieId, roomId) => {
    ...
    // Proceed with booking logic here if user is logged in
    router.push(`/booking/${movieId}/${roomId}/${scheduleId}`);
  };
              ...
              {Array.isArray(schedules) && schedules.length > 0 ? (
                schedules.map((schedule) => (
                  <div key={schedule.schedule_id} className="bg-white rounded-lg shadow-md p-5">
                    ...
                    <button
                      onClick={() => handleBooking(schedule.schedule_id, schedule.movie_id, schedule.room_id)}
                      className="w-full mt-4 bg-gradient-to-r from-pink-500 to-red-500 text-white py-2 rounded-lg hover:bg-red-600 transition"
                    >
                      Đặt vé
                    </button>
